...
bool valid_rev(int x, int y, int dist, int dir) {
    if (x < 0 || y < 0 || x >= n || y >= m) {
        return 0;
    }
    if (mat[x][y] == '#') {
...
int main() {
    ...
    int minfin = inf, dirfin = -1;
    for (int p=0;p<4;p++) {
        if (mindist[ex][ey][p] < minfin) {
            minfin = mindist[ex][ey][p];
            dirfin = p;
        }
    }

    pos.insert({{-minfin, dirfin}, {ex, ey}});
    mat[ex][ey] = 'O';
    while (!pos.empty()) {
        int dist = pos.begin()->first.first;
        int dir = pos.begin()->first.second;
        ...
        for (int p=0;p<4;p++) {
            if ((p+2)%4 == dir) continue;
            if (valid_rev(x+dx[(dir+2)%4], y+dy[(dir+2)%4], dist+1+1000*(p!=dir), p)) {
                pos.insert({{dist+1+1000*(p!=dir), p}, {x+dx[(dir+2)%4], y+dy[(dir+2)%4]}});
                mat[x+dx[(dir+2)%4]][y+dy[(dir+2)%4]] = 'O';
            }
        }
    }
    int tot = 0;
