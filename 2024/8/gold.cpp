    ...
    for (auto &k: antennas) {
        for (int i=0;i<k.second.size();i++) {
            for (int j=i+1;j<k.second.size();j++) {
                ...
                off_i = (k.second[j].first-k.second[i].first);
                nj = k.second[j].second+(k.second[j].second-k.second[i].second);
                off_j = (k.second[j].second-k.second[i].second);
                while (valid(ni, nj)) {
                    v[ni][nj] = '#';
                    ni += off_i;
                    nj += off_j;
